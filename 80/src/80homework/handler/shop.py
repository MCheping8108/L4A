...
def cart():
    ...
    total = 0
    # 查询用户未支付的购物车记录
    record = list(db.cart.find({'user_id': ObjectId(user['_id']), 'status': 0}))
    # 遍历购物车记录
    for r in record:
        # 把商品详情和商品小计金额新增到购物车记录中
        r['info'] = db.product.find_one({'_id': r['product_id']})
        r['total_price'] = r['info']['price'] * r['amount']
        # 总金额累加商品小计金额
        total += r['total_price']
    # 返回响应：渲染cart.html，返回用户信息user,购物车商品信息record,购物车商品总金额total
    return render_template('cart.html', user=user, record=record, total=total)
...
def pay_cart():
    ...
    return jsonify({'code': 0, 'data': '支付成功'})


@shop.route('/del_cart', methods=['GET'])
def del_cart():
    # 在下方写你的代码：获取购物车商品记录id，删除该商品记录
    pass
    # 返回响应：状态 code 为 0，提示信息 data 为“删除成功”
