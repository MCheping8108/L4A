...
def microblog_find(page, count=5, type='paging'):
    ...
    :param page: 页数
    :param count: 每页显示数量
    :param type: 类型，paging表示获取分页数据，init表示获取前page页的所有数据
    :return: 根据 page 返回列表 microblogs 中的对象
    '''
    data = None

    if type == 'paging':
        data = list(db.microblog.find().sort('create_time', -1).skip((page - 1) * count).limit(count))
    elif type == 'init':
        data = list(db.microblog.find().sort('create_time', -1).limit(page * count))
